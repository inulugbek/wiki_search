...
Future<bool> checkNetwork() => NetworkInfo().isConnected;

///Current platform network Info
class NetworkInfo {
  final DataConnectionChecker _connectionChecker = DataConnectionChecker();

  Future<bool> get isConnected => _connectionChecker.hasConnection;

  Stream<bool> get isConnectedStream => _connectionChecker.onStatusChange.map(
        (status) {
...
class DataConnectionChecker {
  ...
  static const int kDefaultPort = 53;
  ...
  static const Duration kDefaultInterval = Duration(seconds: 10);
  ...
  Future<bool> get hasConnection async {
    ...
    _lastTryResults = List.unmodifiable(await Future.wait<void>(requests));
  ...
  ///
  /// Defaults to [kDefaultInterval] (10 seconds).
  Duration checkInterval = kDefaultInterval;
...
/// default to [DataConnectionChecker.kDefaultPort]
/// and [DataConnectionChecker.kDefaultTimeout]
...
class AddressCheckOptions {
  ...
  AddressCheckOptions(
    this.address, {
    this.port = DataConnectionChecker.kDefaultPort,
    this.timeout = DataConnectionChecker.kDefaultTimeout,
  });
